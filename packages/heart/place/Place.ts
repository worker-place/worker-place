...
export class Place {
  ...
  async loadImage() {
    for (let i = 0; i < 384; i++) {
      ...
      if (!piece) {
        ...
        this.mem.IMAGE?.fill(4, i * 8192, (i + 1) * 8192)
      } else {
        this.mem.IMAGE?.set(piece, i * 8192)
      }
    }
  }
  ...
  async saveImage() {
    const writes = []
    for (let i = 0; i < 384; i++) {
      const piece = i === 383 ? this.mem.IMAGE?.subarray(i * 8192) : this.mem.IMAGE?.subarray(i * 65000, (i + 1) * 8192)
